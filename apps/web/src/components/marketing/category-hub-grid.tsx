    ...
    >
      {categories.map((category) => (
        <motion.div key={category.href} variants={item}>
          <Link
            href={category.href}
            className="group flex h-full flex-col rounded-xl border border-neutral-200 bg-white p-5 shadow-sm transition-all motion-safe:hover:-translate-y-0.5 hover:border-brand-deep/30 hover:shadow-md"
          >
